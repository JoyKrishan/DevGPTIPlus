    ...
    for item in data:
        ...
        if any(True for sharing in sharings if sharing and 'Model' in sharing):
            if language not in desired_languages:
                print(f"Skipping issue {item['issue_URL']} with language {language} not in desired languages.")
            if language:
...
def create_visualizations(length_stats, context_stats, code_stats):
    """Create visualizations for all three analyses"""
    import matplotlib.pyplot as plt
    import numpy as np
    
    # Set up the figure with subplots
    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    fig.suptitle('ChatGPT Dataset Analysis', fontsize=16, fontweight='bold')
    
    # 1. Conversation Length Box Plot
    ax1 = axes[0, 0]
    box_data = [length_stats['prompt_lengths']['data'], 
                length_stats['answer_lengths']['data'],
                length_stats['prompt_answer_ratios']['data']]
    ax1.boxplot(box_data, labels=['Prompt\nLength', 'Answer\nLength', 'Answer/Prompt\nRatio'])
    ax1.set_title('Conversation Length Distribution')
    ax1.set_ylabel('Characters/Ratio')
